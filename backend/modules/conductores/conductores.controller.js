...
const listar = async (req, res) => {
  try { res.status(200).json(await service.listarConductores()); }
    ...
    console.error('[conductores] listar:', error.message);
    res.status(500).json({ ok: false, data: null, mensaje: 'Error al listar' });
  }
};
...
const listarDisponibles = async (req, res) => {
  try { res.status(200).json(await service.listarUsuariosDisponibles()); }
    ...
    console.error('[conductores] listarDisponibles:', error.message);
    res.status(500).json({ ok: false, data: null, mensaje: 'Error al buscar disponibles' });
  }
};
...
const crear = async (req, res) => {
  try { res.status(201).json(await service.registrarConductor(req.body)); }
    ...
    console.error('[conductores] crear:', error.message);
    res.status(500).json({ ok: false, data: null, mensaje: 'Error al registrar' });
  }
};
...
const editar = async (req, res) => {
  try { res.status(200).json(await service.modificarConductor(req.params.id, req.body)); }
    ...
    console.error('[conductores] editar:', error.message);
    res.status(500).json({ ok: false, data: null, mensaje: 'Error al actualizar' });
  }
};
...
const cambiarEstado = async (req, res) => {
  try { res.status(200).json(await service.alternarEstado(req.params.id, req.body.estado)); }
    ...
    console.error('[conductores] cambiarEstado:', error.message);
...
const eliminar = async (req, res) => {
  try { res.status(200).json(await service.eliminarConductor(req.params.id)); }
    ...
    console.error('[conductores] eliminar:', error.message);
