...
const MonitoreoTurnos = () => {
  ...
  const [conductorSeleccionado, setConductorSeleccionado] = useState(null); // guarda ConductorID
  ...
  // Filtra conductores disponibles — usa ConductorID en todas las comparaciones
  const getConductoresDisponibles = () => {
    const ocupados = turnos.map(t => Number(t.ConductorID));
    return todosLosConductores.filter(c => {
      if (busquedaConductor && !c.NombreCompleto.toLowerCase()
          .includes(busquedaConductor.toLowerCase())) return false;
      if (ocupados.includes(Number(c.ConductorID))) return false;
    ...
    setModalAbierto({
      ...
      accion: async () => {
        try {
          ...
          setToast({ mensaje: 'Error al cerrar turnos', tipo: 'error' });
        }
      }
    });
  };
  ...
  const ejecutarReasignacion = async () => {
    ...
    try {
      ...
      setToast({ mensaje: 'Error en la reasignación', tipo: 'error' });
    }
  };
  ...
  return (
    <div style={{ height: 'calc(100vh - 90px)', width: '100%', boxSizing: 'border-box', display: 'flex', flexDirection: 'column', padding: '24px', overflow: 'hidden' }}>
      ...
      <div style={{ flexShrink: 0 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
          <h1 style={{ fontSize: '2rem', fontWeight: '800', display: 'flex', alignItems: 'center', gap: '12px' }}>
            <Activity color="var(--primary)" /> Monitoreo de Turnos Activos
          </h1>
          <button onClick={cargarDatos} style={{ padding: '8px 16px', borderRadius: '8px', border: '1px solid var(--border)', background: 'white', fontWeight: '600', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px' }}>
        ...
        {seleccionados.length > 0 && (
          <div style={{ background: '#eff6ff', border: '1px solid #bfdbfe', padding: '12px 24px', borderRadius: '12px', marginBottom: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontWeight: '700', color: '#1e3a8a', display: 'flex', alignItems: 'center', gap: '8px' }}>
              <CheckSquare size={20} /> {seleccionados.length} turno(s) seleccionado(s)
            </span>
            <button onClick={() => confirmarCierre(seleccionados)} style={{ padding: '8px 16px', borderRadius: '8px', background: '#dc2626', color: 'white', border: 'none', fontWeight: '700', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px' }}>
              <XOctagon size={16} /> Forzar Cierre Masivo
            </button>
          </div>
        )}
      </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', position: 'relative' }}>
            <thead style={{ position: 'sticky', top: 0, zIndex: 10, background: '#f8fafc', boxShadow: '0 1px 2px rgba(0,0,0,0.05)' }}>
              <tr style={{ textAlign: 'left', color: 'var(--text-muted)' }}>
                <th style={{ padding: '16px', width: '40px', textAlign: 'center', borderBottom: '1px solid var(--border)' }}>
                  <input type="checkbox"
                    checked={seleccionados.length === turnosActuales.length && turnosActuales.length > 0}
                    onChange={(e) => setSeleccionados(e.target.checked ? turnosActuales.map(t => t.TurnoConductorID) : [])}
            ...
            <tbody>
              {turnos.length === 0 ? (
                <tr>
                  <td colSpan="6" style={{ padding: '32px', textAlign: 'center', color: 'var(--text-muted)', fontSize: '1.1rem' }}>
                    No hay autobuses operando actualmente.
                  </td>
                </tr>
              ) : (
                turnosActuales.map(t => (
                  <tr key={t.TurnoConductorID} style={{ borderBottom: '1px solid var(--border)', background: seleccionados.includes(t.TurnoConductorID) ? '#eff6ff' : 'white' }}>
                    <td style={{ padding: '16px', textAlign: 'center' }}>
                      <input type="checkbox"
                        checked={seleccionados.includes(t.TurnoConductorID)}
                        onChange={() => setSeleccionados(prev =>
                          prev.includes(t.TurnoConductorID)
                            ? prev.filter(id => id !== t.TurnoConductorID)
                            : [...prev, t.TurnoConductorID]
                        )}
                        style={{ cursor: 'pointer' }}
                      />
                    </td>
                    ...
                    <td style={{ padding: '16px', textAlign: 'center' }}>
                      <div style={{ display: 'flex', gap: '8px', justifyContent: 'center' }}>
                        <button
                          onClick={() => {
                            setModalReasignar({ mostrar: true, turnoId: t.TurnoConductorID });
                            setConductorSeleccionado(null);
        ...
        {!cargando && turnos.length > 0 && (
          <div style={{ padding: '16px 24px', background: 'white', borderTop: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 }}>
            ...
            <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
              <button onClick={() => setPaginaActual(p => Math.max(1, p - 1))} disabled={paginaActual === 1} style={{ padding: '8px', borderRadius: '8px', border: '1px solid var(--border)', cursor: paginaActual === 1 ? 'not-allowed' : 'pointer', background: paginaActual === 1 ? '#f8fafc' : 'white' }}>
                <ChevronLeft size={18} />
              </button>
              <span style={{ fontSize: '14px', fontWeight: '600' }}>Página {paginaActual} de {totalPaginas}</span>
              <button onClick={() => setPaginaActual(p => Math.min(totalPaginas, p + 1))} disabled={paginaActual === totalPaginas} style={{ padding: '8px', borderRadius: '8px', border: '1px solid var(--border)', cursor: paginaActual === totalPaginas ? 'not-allowed' : 'pointer', background: paginaActual === totalPaginas ? '#f8fafc' : 'white' }}>
      ...
      {modalReasignar.mostrar && (
        <div className="modal-overlay" style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1100, display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
          <div style={{ background: 'white', padding: '32px', borderRadius: '16px', maxWidth: '500px', width: '90%', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.25)' }}>
            ...
            <div style={{ position: 'relative', marginBottom: '16px' }}>
              ...
              <input
                ...
                style={{ width: '100%', padding: '10px 10px 10px 38px', borderRadius: '8px', border: '1px solid var(--border)', fontSize: '14px', boxSizing: 'border-box' }}
              />
            </div>
            ...
            <div style={{ border: '1px solid var(--border)', borderRadius: '8px', maxHeight: '250px', overflowY: 'auto', marginBottom: '24px' }}>
              {getConductoresDisponibles().length === 0 ? (
                <div style={{ padding: '24px', textAlign: 'center', color: 'var(--text-muted)' }}>
                  No hay conductores disponibles actualmente.
                </div>
              ) : (
                getConductoresDisponibles().map(c => {
                  const vencida   = validarLicenciaVencida(c.VencimientoLicencia);
                  const estaSelec = Number(conductorSeleccionado) === Number(c.ConductorID);
                  ...
                  return (
                    <div
                      ...
                      onClick={() => {
                        if (vencida) {
                          setModalAbierto({
                            mostrar: true, soloAlerta: true,
                            titulo: 'Atención: Licencia Vencida',
                            mensaje: `No puedes asignar a ${c.NombreCompleto} porque su licencia venció el ${formatearFechaCorta(c.VencimientoLicencia)}.`,
                            colorBoton: '#dc2626', textoBoton: 'Entendido',
                            accion: () => setModalAbierto({ mostrar: false })
                          });
                        } else {
                          setConductorSeleccionado(Number(c.ConductorID));
                        }
                      }}
                      style={{
                        ...
                        cursor: 'pointer',
                        background: estaSelec ? '#eff6ff' : 'white',
                        opacity: vencida ? 0.6 : 1,
                        transition: 'background 0.2s'
                      }}
                    >
                      <div style={{
                        width: '36px', height: '36px', borderRadius: '50%', flexShrink: 0,
                        background: estaSelec ? 'var(--primary)' : '#f1f5f9',
                        color: estaSelec ? 'white' : '#64748b',
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontSize: '12px', color: vencida ? '#dc2626' : '#64748b', fontWeight: vencida ? '700' : '400' }}>
                          Vence: {formatearFechaCorta(c.VencimientoLicencia)} {vencida && '(VENCIDA)'}
                        </div>
                      </div>
                    </div>
                  );
            ...
            <div style={{ display: 'flex', gap: '12px' }}>
              <button
                onClick={() => { setModalReasignar({ mostrar: false, turnoId: null }); setConductorSeleccionado(null); }}
                style={{ flex: 1, padding: '12px', background: '#f1f5f9', borderRadius: '10px', border: 'none', fontWeight: '700', cursor: 'pointer', color: '#475569' }}
              >
                Cancelar
              </button>
              <button
                ...
                style={{ flex: 1, padding: '12px', background: 'var(--primary)', color: 'white', borderRadius: '10px', border: 'none', fontWeight: '700', opacity: conductorSeleccionado ? 1 : 0.5, cursor: conductorSeleccionado ? 'pointer' : 'not-allowed' }}
              >
      ...
      {modalAbierto.mostrar && (
        <div className="modal-overlay" style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1200, display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
          <div style={{ background: 'white', padding: '32px', borderRadius: '16px', maxWidth: '450px', width: '90%', textAlign: 'center', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.25)' }}>
            ...
            <div style={{ display: 'flex', gap: '12px' }}>
              {!modalAbierto.soloAlerta && (
                <button onClick={() => setModalAbierto({ mostrar: false })} style={{ flex: 1, padding: '12px', background: '#f1f5f9', borderRadius: '10px', border: 'none', fontWeight: '700', cursor: 'pointer', color: '#475569' }}>
                  Cancelar
                </button>
              )}
              <button onClick={modalAbierto.accion} style={{ flex: 1, padding: '12px', background: modalAbierto.colorBoton, color: 'white', borderRadius: '10px', border: 'none', fontWeight: '700', cursor: 'pointer' }}>
                {modalAbierto.textoBoton}
              </button>
            </div>
          </div>
        </div>
